...
# bumps = np.array([0, 0, 0, 1, 2, 5, 5])
# chi_b_init_guesses = np.array([2, 2, 5, 5, 10, 15, 15])
...
# while keep_changing.any() and i < 50:
...
#     print "Iteration: ", i
#     for b in xrange(J):
#         if (chi_fits_old[2*b] + chi_fits_old[2*b + 1])/2.0 < .3:
...
copy_of_theta_tax = np.copy(theta_tax)

for i in xrange(50):
    theta_tax = copy_of_theta_tax * (i+1)/50
    dictionary = {}
    for key in var_names:
        dictionary[key] = globals()[key]
    pickle.dump(dictionary, open("OUTPUT/given_params.pkl", "w"))
